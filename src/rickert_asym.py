...
def rickert_asym(N, d, vmax, cell_length, p, p_change, num_of_iterations=30):
    """
...
def compare():
    ...
    for _ in range(x):
        ...
        for i in range(0, len(flow_arr_1)):
            [flow_1, iterations] = rickert_asym(N=1000, d=density_arr[i], vmax=5, cell_length=7.5, p=0.2, p_change=1)
            [flow_2, iterations] = rickert_asym(N=1000, d=density_arr[i], vmax=5, cell_length=1, p=0.2, p_change=1)
            [flow_3, iterations] = rickert_asym(N=1000, d=density_arr[i], vmax=5, cell_length=0.5, p=0.2, p_change=1)
            #[flow_4, iterations] = rickert_asym(N=1000, d=density_arr[i], vmax=5, cell_length=1, p=0.2, p_change=0.2)
            flow_arr_1[i] += flow_1
            flow_arr_2[i] += flow_2
            flow_arr_3[i] += flow_3
            #flow_arr_4[i] += flow_4
    ...
    comprasion.fundamental_diagram_comprasion(flow_arr_2, density_arr, "l = 1")
    comprasion.fundamental_diagram_comprasion(flow_arr_3, density_arr, "l = 0.5")
    #comprasion.fundamental_diagram_comprasion(flow_arr_4, density_arr, "l = 1, p_change = 0.2")

    plt.legend()
    plt.savefig("rickert_asym_length.png")
